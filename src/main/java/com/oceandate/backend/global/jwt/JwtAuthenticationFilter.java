...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String accessToken = cookieUtil.getCookie(request, ACCESS_COOKIE_NAME)
                .map(Cookie::getValue)
                .orElse(null);

        if (accessToken == null) {
            accessToken = resolveTokenFromHeader(request);
        }
        ...
        if (accessToken != null && jwtTokenValidator.validateToken(accessToken)) {
            ...
            log.debug("JWT 인증 성공 - 사용자 ID: {}",
                    ((AccountContext) authentication.getPrincipal()).getMemberId());
        }
